...
class DisplayAction(Node):
    ...
    def render(self, context):
        ...
        try:
            template_name = 'activity/%(verb)s/action.html' % {
                'verb': action_instance.verb.replace(' ', '_') 
            }
            action_output = render_to_string(template_name, {
                'action': action_instance
            }, context)
        except TemplateDoesNotExist:
            template_name = 'activity/action.html'
            action_output = render_to_string(template_name, {
                'action': action_instance
            }, context)
        if self.varname is not None:
...
class DisplayActionShort(Node):
    ...
    def render(self, context):
        ...
        try:
            action_output = render_to_string(template_name, {
                'hide_actor': True, 'action': action_instance
            }, context)
        except TemplateDoesNotExist:
            template_name = 'activity/action.html'
            action_output = render_to_string(template_name, {
...
class DisplayGroupedActions(Node):
    ...
    def render(self, context):
        ...
        try:
            template_name = 'activity/%(verb)s/grouped.html' % {
                'verb': actions_instance[0].verb
            }
            action_output = render_to_string(template_name, {
                'actions': actions_instance
            })
        except TemplateDoesNotExist:
            template_name = 'activity/grouped.html'
            action_output = render_to_string(template_name, { 
                'actions': actions_instance
            })
        if self.varname is not None:
    ...
    if len(bits) > 3:
        if len(bits) != 4:
            raise TemplateSyntaxError(
                'Accepted formats {% display_action [action] %}\
 or {% display_action [action] as [var] %}')
        if bits[2] != 'as':
            raise TemplateSyntaxError( 
                'Accepted formats {% display_action [action] %}\
 or {% display_action [action] as [var] %}')
...
def do_print_grouped_actions(parser, token):
    ...
    if len(bits) > 3:
        if len(bits) != 4:
            raise TemplateSyntaxError(
                'Accepted formats {% display_grouped_actions [action] %}\
 or {% display_action [action] as [var] %}')
        if bits[2] != 'as':
            raise TemplateSyntaxError(
                'Accepted formats {% display_grouped_actions [action] %}\
...
def do_print_action_label(parser, token):
    ...
    if len(bits) > 3:
        if len(bits) != 4:
            raise TemplateSyntaxError('Accepted formats\
 {% action_label [action] %} or {% action_label [action] as [var] %}')
        if bits[2] != 'as':
            raise TemplateSyntaxError('Accepted formats\
 {% action_label [action] %} or {% action_label [action] as [var] %}')
